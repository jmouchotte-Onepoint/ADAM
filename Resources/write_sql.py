from Resources.utils import esc, sql_value, sql_array, sql_jsonb
...
def init_row_level(policy_name, policy_table_name, adam_table, policy_action):  
    ...
    {policy_clause} (
        EXISTS (
            SELECT 1 FROM {adam_table} AS rrp
...
def add_new_row_level_in_list(policy, policy_table_name, policy_object_type, adam_table, relation):
    ...
    if policy.get("attributes",False) == True:
        data += f"""\n\t\t\tAND fn_check_all_attributes_from_view(rrp.id::UUID, '{adam_table}', get_current_user_attributes())"""
    # Ajoute les règles si rules active
    if policy.get("rules",False) == True:
        data += f"""\n\t\t\tAND fn_check_all_rules_from_view(rrp.id::UUID, '{adam_table}', get_current_user_attributes())"""
        ...
        for item in array_json_conditions:
            for key, value in item.items():
                if "like" in str(value):
                    ...
                    lines.append(f"{app_table_name}.{key} = {sql_value(value)}")
...
def write_model(file_path, relation_type, user_type, relation_source, relation_source_type, object_type, relation, relation_origin=None, condition=None):   
    model_id = uuid7(); 
    data = ("INSERT INTO model (id, relation_type, user_type, relation_source, object_type, relation, relation_origin, condition, relation_source_type) "
...
def write_model_rules(file_path, model_id, list_rules): 
    ...
    for rule in list_rules:
        data += ("INSERT INTO model_rules (model_id, rule_id, params) "
            f"VALUES ({sql_value(model_id)},{sql_value(rule.get('id'))}, {sql_jsonb(rule.get('params'))});")
    #Sauvegarde dans le fichier SQL
    if data != '':
        write_sql(file_path, data)
    
def write_sql_data_relations(file_path, user_type, user_id, relation, object_type, object_id, tenant_id, table='relations'):
    data = (f"INSERT INTO {table} (user_type,user_id, relation, object_type, object_id, tenant_id) "
            f"VALUES ('{esc(user_type)}', '{esc(user_id)}','{esc(relation)}', '{esc(object_type)}', '{esc(object_id)}', '{esc(tenant_id)}');")
    write_sql(file_path, data)
    
def write_sql_data_roles(file_path, user_type, user_id, relation, object_type, object_id, tenant_id, table='roles'):
    data = (
        f"INSERT INTO {table} (user_type,user_id, relation, object_type, object_id, tenant_id, model_id) "
        f"VALUES ('{esc(user_type)}', '{esc(user_id)}','{esc(relation)}', '{esc(object_type)}', '{esc(object_id)}', '{esc(tenant_id)}', "
        f"(SELECT id FROM model WHERE relation_type = 'ROLES' AND user_type = '{esc(user_type)}' AND relation = '{esc(relation)}' AND object_type = '{esc(object_type)}' LIMIT 1));"
    )
    write_sql(file_path, data)

def write_sql_data_tenants(file_path, tenant_id, name, description):
    data = (f"INSERT INTO tenants (id, name, description) "
...
def write_sql_rules(file_path, id, rule, params, description):
    data = (f"INSERT INTO dynamic_rules (id, rule, params, description) "
